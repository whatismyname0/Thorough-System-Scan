...
public class TSScan_SalvageReportIntel extends BaseIntelPlugin {
    ...
    protected void addBulletPoints(TooltipMakerAPI info, ListInfoMode mode, boolean isUpdate, Color tc, float initPad) {
        ...
        if (itemCount.get(Items.TAG_MODSPEC) > 0) {
            info.addPara("%s 个舰船插件蓝图", initPad, tc, h, String.format("%.2f",itemCount.get(Items.TAG_MODSPEC)));
            initPad = 0f;
        }
        if (itemCount.get(Items.TAG_COLONY_ITEM) > 0) {
            info.addPara("%s 个殖民地工业用特殊物品", initPad, tc, h, String.format("%.2f",itemCount.get(Items.TAG_COLONY_ITEM)));
            initPad = 0f;
        }
